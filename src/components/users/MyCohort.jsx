...
const MyCohort = () => {
  ...
  const [usersByDate, setUsersByDate] = useState({ [todayDateKey]: [] });
  ...
  const fetchTodayUsers = async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://qalib.cloud/api/admin/users-by-date");
      const data = response.data || {};
      const grouped = data.usersByDate || {};

      setUsersByDate({ [todayDateKey]: grouped[todayDateKey] || [] });
    } catch (error) {
      console.error("Error fetching today users:", error);
      setUsersByDate({ [todayDateKey]: [] });
    } finally {
      setLoading(false);
      setCurrentPage(1);
    }
  ...
  const todayUsers = usersByDate[todayDateKey] || [];
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = todayUsers.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.max(1, Math.ceil(todayUsers.length / itemsPerPage));
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <h1 className="text-3xl font-bold text-center text-gray-800 mt-6">Your cohort</h1>

      <div className="flex justify-between">
        <div className="flex items-center gap-4 mt-10 mb-3">
          <Items name={`Today Users (${todayUsers.length})`} />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-10 text-center mt-4">
          <p className="text-gray-600">No users registered today</p>
        </div>
      )}
